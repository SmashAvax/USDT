...
let web3, smashnadoAddress, smashnado, circuit, proving_key, groth16, erc20, senderAccount, netId
...
/** Display USDT account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} USDT balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}

/** Display ERC20 account balance */
async function printERC20Balance({ address, name, tokenAddress }) {
  erc20ContractJson = require('./build/contracts/IERC20.json')
  erc20 = tokenAddress ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : erc20
  console.log(`${name} Token Balance is`, web3.utils.fromWei(await erc20.methods.balanceOf(address).call(), 'mwei'))
}
...
async function deposit({ currency, amount }) {
  ...
  console.log('Current allowance is', fromWei(allowance, 'mwei'))
  if (toBN(allowance).lt(toBN(tokenAmount))) {
    console.log('Approving tokens for deposit')
    w_display_loader_message('Approving tokens for deposit')
    const toHex = "0x" + tokenAmount.toString(16);
    await erc20.methods.approve(smashnado._address, toHex).send({ from: senderAccount, gas: 1e6 })
  }

  console.log('Submitting deposit transaction')
  w_display_loader_message('Submitting deposit transaction')
  // to avoid lost security key !
  download_note('secret_key_' + currency + '_' + amount + '.txt', noteString)

  const deposited = await smashnado.methods.deposit(toHex(deposit.commitment)).send({ from: senderAccount, gas: 2e6 })
  await printERC20Balance({ address: smashnado._address, name: 'Smashcash' })
  await printERC20Balance({ address: senderAccount, name: 'Sender account' })
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'USDT' && refund !== '0') {
    throw new Error('The USDT purchase is supposted to be 0 for USDT withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 1 || netId === 42) {
        console.log(`Transaction submitted through the relay. View transaction on etherscan https://etherscan.com//tx/${relay.data.txHash}`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 1 || netId === 42) {
          console.log(`View transaction on https://etherscan.com/tx/${txHash}`)
        } else {
...
async function loadDepositData({ deposit }) {
  try {
    const eventWhenHappened = await smashnado.getPastEvents('Deposit', {
      filter: {
    ...
    const isSpent = await smashnado.methods.isSpent(deposit.nullifierHex).call()
...
async function loadWithdrawalData({ amount, currency, deposit }) {
  try {
    const events = await await smashnado.getPastEvents('Withdrawal', {
      fromBlock: 0,
      toBlock: 'latest'
    })
...
function process_errormsg(error) {
  ...
  if (typeof error === 'object') {
    if (error.message.includes('Internal JSON-RPC')) {
      ...
      if (error.message.toLowerCase().includes('rpc')) {
        w_display_notification('There is problem with your RPC. If you set correct RPC, retry later or reload page', 'error')
      } else {
        w_display_notification(error.message)
      }            
    }  
  } else {
    w_display_notification(error)
  }
}
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/USDTSmashnado.json')).json()
    ...
    if (web3 === undefined) {
      ...
      contractJson = require('./build/contracts/USDTSmashnado.json')
      ...
      erc20ContractJson = require('./build/contracts/ERC20Mock.json')
      erc20smashnadoJson = require('./build/contracts/ERC20Smashnado.json')
    }
  }
  ...
  if (!(netId === 1 || netId === 42)) {
    throw new Error('You did not connect to Ethereum network. Please make sure you connected to Ethereum Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to Ethereum Mainnet.')
  erc20 = currency !== 'usdt' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'usdt', amount: '0.1' }
    //await init(instance)
